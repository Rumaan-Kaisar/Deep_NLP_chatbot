...
lines_in_cnvrstn = open("../data_small_50_movie/movie_lines.txt", encoding='utf-8', errors='ignore').read().split('\n')
cnvrstn = open("../data_small_50_movie/movie_conversations.txt", encoding='utf-8', errors='ignore').read().split('\n')
...
early_stopping_stop = 100      # we choose 1000 to reach the last epoch i.e. 100-th epoch. Generally we set early_stopping 100
...
while(True):
    ...
    if question == 'Goodbye':
        break
    question = convert_string2int(question, questnWrd2Int)
    question = question + [questnWrd2Int['<PAD>']] * (25 - len(question))
    fake_batch = np.zeros((batch_size, 25))
    fake_batch[0] = question
    predicted_answer = session.run(test_predictions, {inputs: fake_batch, keep_prob: 0.5})[0]
    answer = ''
    for i in np.argmax(predicted_answer, 1):
        if ans_Int_2_Wrd[i] == 'i':
            token = ' I'
        elif ans_Int_2_Wrd[i] == '<EOS>':
            token = '.'
        elif ans_Int_2_Wrd[i] == '<OUT>':
            token = 'out'
        else:
            token = ' ' + ans_Int_2_Wrd[i]
        answer += token
        if token == '.':
            break
